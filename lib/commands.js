...
const queryFilter = curry((query, item) => {
  console.log(item['aid_skill'].indexOf(buffList[query]) >= 0 );
...
const getResult = (msg) => {
  ...
  const items = isWeapon(category) ? 
                 Object.keys(requestAllItem[category]).map( key => requestAllItem[category][key]) :
                 requestAllItem;
  ...
  if(searchtList.includes(query_1)) {
